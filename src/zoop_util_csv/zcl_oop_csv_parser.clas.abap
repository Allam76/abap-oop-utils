class ZCL_OOP_CSV_PARSER definition
  public
...
public section.
*"* public components of class ZCL_OOP_CSV_PARSER
*"* do not include other source files here!!!
  type-pools ABAP .

  methods CONSTRUCTOR
    importing
      !DELEGATE type ref to ZIF_oop_CSV_PARSER_DELEGATE
      !CSVSTRING type STRING
      !SEPARATOR type C
      !SKIP_FIRST_LINE type ABAP_BOOL .
  methods PARSE
    raising
      ZCX_oop_CSV_PARSE_ERROR .
protected section.
*"* protected components of class ZCL_OOP_CSV_PARSER
*"* do not include other source files here!!!
private section.

*"* private components of class ZCL_OOP_CSV_PARSER
*"* do not include other source files here!!!
  constants _TEXTINDICATOR type C value '"'. "#EC NOTEXT
  data _DELEGATE type ref to ZIF_oop_CSV_PARSER_DELEGATE .
...
CLASS ZCL_OOP_CSV_PARSER IMPLEMENTATION.


* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_OOP_CSV_PARSER->CONSTRUCTOR
* +-------------------------------------------------------------------------------------------------+
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_OOP_CSV_PARSER->PARSE
* +-------------------------------------------------------------------------------------------------+
* | [!CX!] zcx_opp_csv_parse_error
* +--------------------------------------------------------------------------------------</SIGNATURE>
method parse.
  data msg type string.
  if _csvstring is initial.
    message e002(zcsv) into msg.
    raise exception type zcx_oop_csv_parse_error
      exporting
        message = msg.
  endif.
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Private Method ZCL_OOP_CSV_PARSER->_LINES
* +-------------------------------------------------------------------------------------------------+
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Private Method ZCL_OOP_CSV_PARSER->_PARSE_LINE
* +-------------------------------------------------------------------------------------------------+
* | [--->] LINE                           TYPE        STRING
* | [<-()] RETURNING                      TYPE        STRINGTAB
* | [!CX!] zcx_opp_csv_parse_error
