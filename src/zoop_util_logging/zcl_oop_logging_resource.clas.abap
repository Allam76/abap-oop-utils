class ZCL_OOP_LOGGING_RESOURCE definition
  public
  inheriting from ZCL_oop_RESOURCE
  final
  create public .

public section.

*"* public components of class ZCL_OOP_LOGGING_RESOURCE
*"* do not include other source files here!!!
  ...
  methods UPDATE
    redefinition .
protected section.
*"* protected components of class ZCL_OOP_LOGGING_RESOURCE
*"* do not include other source files here!!!
private section.

*"* private components of class ZCL_OOP_LOGGING_RESOURCE
*"* do not include other source files here!!!
  data LOGGING_REPOSITORY type ref to ZCL_oop_LOGGING_REPOSITORY .
...
CLASS ZCL_OOP_LOGGING_RESOURCE IMPLEMENTATION.


* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_OOP_LOGGING_RESOURCE->CONSTRUCTOR
* +-------------------------------------------------------------------------------------------------+
* +--------------------------------------------------------------------------------------</SIGNATURE>
method constructor.
  super->constructor( ).
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_OOP_LOGGING_RESOURCE->CREATE
* +-------------------------------------------------------------------------------------------------+
...
*/
  ...
  if tp_log_object is initial.
    ...
    response->send_error( code = ZCL_oop_HTTP_STATUS_CODES=>bad_request message = msg ).
    return.
  endif.
  ...
  if tp_log_number is initial.
    ...
    response->send_error( code = ZCL_oop_HTTP_STATUS_CODES=>internal_server_error message = msg ).
    return.
  endif.
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Private Method ZCL_OOP_LOGGING_RESOURCE->PARSE_URI
* +-------------------------------------------------------------------------------------------------+
* | [--->] URI                            TYPE        STRING
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_OOP_LOGGING_RESOURCE->READ
* +-------------------------------------------------------------------------------------------------+
* | [--->] REQUEST                        TYPE REF TO ZCL_oop_REQUEST
* | [--->] RESPONSE                       TYPE REF TO ZCL_oop_RESPONSE
* +--------------------------------------------------------------------------------------</SIGNATURE>
...
*/
  ...
  if logger is not bound.
    ...
    response->send_error( code = ZCL_oop_HTTP_STATUS_CODES=>not_found message = msg ).
    return.
  endif.
  ...
  log_object_json = ZCL_oop_JSON_UTIL=>new_pair_with_string( name = 'object' value = log_object_str ).
  log_subobject_json = ZCL_oop_JSON_UTIL=>new_pair_with_string( name = 'subobject' value = log_subobject_str ).
  ...
  response->send_text( data = log_as_string mime_type = zcl_oop_http_mime_types=>text_plain code = ZCL_oop_HTTP_STATUS_CODES=>ok ).
  return.
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_OOP_LOGGING_RESOURCE->UPDATE
* +-------------------------------------------------------------------------------------------------+
* | [--->] REQUEST                        TYPE REF TO ZCL_oop_REQUEST
* | [--->] RESPONSE                       TYPE REF TO ZCL_oop_RESPONSE
* +--------------------------------------------------------------------------------------</SIGNATURE>
