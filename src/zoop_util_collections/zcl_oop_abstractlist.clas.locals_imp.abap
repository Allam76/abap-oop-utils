...
*
*----------------------------------------------------------------------*
class lcl_iterator definition inheriting from zcl_oop_object.
  public section.
    interfaces zif_oop_iterator.
    methods constructor importing enclosinglist type ref to zcl_oop_abstractlist.
  protected section.
    data enclosinglist type ref to zcl_oop_abstractlist.
...
class zcl_oop_abstractlist definition local friends lcl_iterator lcl_listiterator.
...
class lcl_iterator implementation.
  ...
  method zif_oop_iterator~next.
    ...
    try.
        ...
        returning = next.
        return.
      catch zcx_oop_indexoutofbounds.
        me->checkforcomodification( ). " Check if this exception is caused by a concurrent modification
        raise exception type zcx_oop_nosuchelement. " Not a concurrent modification, element simply not found
    endtry.
  endmethod.                    "zif_oop_iterator~NEXT
  method zif_oop_iterator~remove.
    if lastret = -1.
      raise exception type zcx_oop_illegalstate.
    endif.
    me->checkforcomodification( ).
    try.
        ...
        raise exception type zcx_oop_concurrmodification.
    endtry.
...
class lcl_listiterator implementation.
  ...
  method zif_oop_listiterator~previous.
    ...
    try.
        ...
        data previous type ref to zcl_oop_object.
